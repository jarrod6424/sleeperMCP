...
def get_league(
    ...
    if not data:
        return {"error": "league not found", "league_id": lid}
...
def get_my_team(
    ...
    return _league_mod.team_report(
        lid, resolved["roster"], resolved["owner"], resolved["matched_by"], players
    )
...
def scout_team(
    ...
    if not resolved:
        users = _http.get_json(f"/league/{lid}/users") or []
        available = [
...
def recent_moves(
    ...
    for wk in range(current, max(0, current - weeks), -1):
        if wk < 1:
            break
        moves.extend(_league_mod.compute_transactions(lid, wk))
...
def search_player(name: str, limit: int = 10) -> list[dict]:
    ...
    for pid, info in players.items():
        ...
        if needle in full:
            matches.append(_players.player_name(pid, players))
...
def get_trending_players(
    ...
    trends = _http.get_json(path) or []
    players = _players.load_players()
    out = []
    for t in trends:
        rec = _players.player_name(t.get("player_id"), players)
...
def get_user(username_or_id: str) -> dict:
    ...
    return _http.get_json(f"/user/{username_or_id}") or {
...
def get_projections(
    ...
    for pid in proj:
        ...
        if pos and rec["position"] != pos:
            continue
        rec["projected_points"] = _proj.proj_points(pid, proj, field)
        rows.append(rec)
...
def get_trade_values(
    ...
    pos = position.upper() if position else None
    rows = [_values.fc_row(v) for v in values]
    if pos:
...
def value_my_roster(
    ...
    players_map = _players.load_players()
    owned = resolved["roster"].get("players") or []
    rows = []
    total = 0
    for pid in owned:
        v = by_sid.get(str(pid))
        rec = _players.player_name(pid, players_map)
        val = (v or {}).get("value")
        if val:
            total += val
        rows.append(
...
def get_adp(
    ...
    return _adp.adp_rows(fmt, season, position=position, limit=limit,
                         fc_values=_values.fc_values(fmt))
...
def get_dynasty_tiers(
    ...
    values = _values.fc_values(dynasty_fmt)
    if not values:
        return {"error": "no trade values returned", "format": dynasty_fmt, "source": FC_SOURCE}

    pos = position.upper() if position else None
    rows = [_values.fc_row(v) for v in values if v.get("value")]
    if pos:
        rows = [r for r in rows if r["position"] == pos]
...
def get_player_stats(
    ...
    season = season or _stats.current_season()
    rows = _http.nflverse_csv("stats_player", f"stats_player_week_{season}.csv",
                          ttl=STATS_CACHE_TTL)
    if not rows:
        return {"error": "no data", "season": season,
                "hint": "Season data may not be available yet."}

    matched = _stats.match_player(rows, player_name, "player_display_name")
    if not matched:
        matched = _stats.match_player(rows, player_name, "player_name")
    if not matched:
    ...
    return {
        ...
        "stats": [_stats.coerce(r, _stats.STAT_KEEP) for r in recent],
    }
...
def get_snap_counts(
    ...
    Source: nflverse (MIT licensed)."""
    season = season or _stats.current_season()
    rows = _http.nflverse_csv("snap_counts", f"snap_counts_{season}.csv",
                          ttl=STATS_CACHE_TTL)
    if not rows:
        return {"error": "no snap data", "season": season}

    matched = _stats.match_player(rows, player_name, "player")
    if not matched:
        return {"error": "player not found", "query": player_name, "season": season}
...
def get_injuries(
    ...
    Source: nflverse (MIT licensed)."""
    season = season or _stats.current_season()
    rows = _http.nflverse_csv("injuries", f"injuries_{season}.csv",
    ...
    out = [
        _stats.coerce(r, inj_keep) for r in filtered
...
def get_team_offense_crowding(
    ...
    season_used, season_label = _offense.stats_season_with_label() if not season else (season, season)
    if season:
        season_used, season_label = season, season

    analysis = _offense.crowding_analysis(team.upper(), season_used)
    if not analysis:
        return {"error": "no data found", "team": team, "season": season_used}

    tiers = _offense.load_playcaller_tiers()
    ...
    return {
        ...
        "play_caller": {
            ...
            **({"config": _offense.playcaller_tiers_meta()}
               if _offense.playcaller_tiers_meta().get("stale") else {}),
        },
        "team_concentration": {
...
def custom_score_player(
    ...
    if player_team or player_name:
        all_rows = _http.nflverse_csv("stats_player", f"stats_player_week_{season_used}.csv",
                              ttl=STATS_CACHE_TTL)
        needle = player_name.strip().lower()
        stat_rows = [
            r for r in all_rows
    ...
    floor_detail: dict = {}
    if stat_rows:
        ppr_pts = [_offense.safe_float(r.get("fantasy_points_ppr")) for r in stat_rows]
        games = len(ppr_pts)
    ...
    if stat_rows:
        ...
        active_weeks = sum(
            ...
            if _offense.safe_float(r.get("fantasy_points_ppr")) > 0
            or _offense.safe_float(r.get("targets")) > 0
            or _offense.safe_float(r.get("carries")) > 0
        )
    ...
    if stat_rows:
        pos = player_pos or ""
        shares = [_offense.safe_float(r.get("target_share")) for r in stat_rows if _offense.safe_float(r.get("target_share")) > 0]
        woprs = [_offense.safe_float(r.get("wopr")) for r in stat_rows if _offense.safe_float(r.get("wopr")) > 0]
        carries = [_offense.safe_float(r.get("carries")) for r in stat_rows]
    ...
    if player_team:
        ...
        analysis = _offense.crowding_analysis(player_team.upper(), season_used)
        if analysis:
            player_entry = next(
        ...
        _pc_meta = _offense.playcaller_tiers_meta()
