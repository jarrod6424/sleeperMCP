...
| ITEM-005 | WR YPRR proxy, NGS catch %, volume | In Review | `yards_per_catch`, `yac_per_reception`, `target_share` from nflverse weekly stats; `yprr` from nflverse participation (yards / on_pass); `reception_perception` from nflverse NGS catch %; artifacts regenerated 2026-08-11 — ARSB/Chase spot-check 15/16 measured (only `ol_pass_block_rank` unsourced); DraftLab Task 5 pending — see `docs/superpowers/specs/2026-08-11-wr-yprr-catch-volume-design.md` + plan `docs/superpowers/plans/2026-08-11-wr-yprr-catch-volume.md` |
