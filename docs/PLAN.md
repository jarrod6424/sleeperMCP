...
| ITEM-005 | WR YPRR proxy, NGS catch %, volume | In Design | Fill `yprr` (yards/routes) + `reception_perception` (NGS catch %); add `yards_per_catch`, `yac_per_reception`, `target_share`; `CEILING_KNOWN_FACTORS.WR` 10→15; OL pass block remains licensed — see `docs/superpowers/specs/2026-08-11-wr-yprr-catch-volume-design.md` |
